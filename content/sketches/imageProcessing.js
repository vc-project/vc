...
function preload() {
  ...
  img_shader = readShader('/vc/sketches/imageProcessing.frag');
...
function setup() {
  ...
  shader(img_shader);
  img_shader.setUniform('texture', img);
  emitTexOffset(img_shader, img, 'texOffset');
...
function draw() {
  ...
  if (enable_shader.checked()) {
    let mask = [-1,-1,-1,-1,8,-1,-1,-1,-1];
    img_shader.setUniform('mask', mask);
  } else {
