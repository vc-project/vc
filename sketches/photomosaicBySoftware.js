...
function preload() {
  ...
  mandrill_image = loadImage('/vc/sketches/mandrill.png'); 
  random_image = loadImage('https://picsum.photos/500');
...
if (enable_shader.checked()) {
  ...
  for (var starty = 0; starty < img.height; starty+=image_size) { // creates pixel index
    for (var startx = 0; startx < img.width; startx+=image_size) {
      ...
      index = floor(lumav(r,g,b)*nimages)

      image(imQuad.read(0,index), startx*width/img.width, starty*height/img.height, batch_size, batch_size)
      //1
      //2 quadrille o graphics get i del quadrille
      //luma v es de 0 a 1 llevarlo a 0,20. floor para obtener indice.
